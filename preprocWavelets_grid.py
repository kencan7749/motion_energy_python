...
def preprocWavelets_grid(S, params):
    ...
    S = S.reshape(np.int(xypixels), stimxytsize[2], order='C')
    ...
    if params.show_or_preprocess:
        if params.phasemode in [5,6,7,8]:
            ...
            if params.phasemode in [5,7]:
                ...
                print('Using only dPhase/dt channels: %d¥n', Spreproc.shape[1])
...
def getGaborParameters(params,aspect_ratio):
    ...
    dir_array = np.arange(0, params.dirdivisions) / params.dirdivisions * 360
    ...
    waveletcount = 0
    gparams = np.zeros([8, 20000], dtype=np.float) #prepare for some amount of memory for gparams
